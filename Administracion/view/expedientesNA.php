            ...
            <div class = "container">
                ...
                <center>
                    <div style ="display: inline-flex; align-content: center; align-items: center;">
                        <h1>Expedientes No Atendidas</h1>&nbsp;&nbsp;&nbsp;
                ...
                <table class = "formulario" align = "center" border = "2">
                    <?php
                        ...
                        $query = "SELECT c.estado, c.nrotramite, t.razonsocial, CONCAT(t.appaterno, ' ', t.apmaterno, ' ', t.nombre) AS nombre_completo, a.nombre AS nombre_asunto, d.nombre AS nombre_dependencia FROM tbtramite c
                    ...
                    <tr align = "center" class = "cuerpo">
                        ...
                        <td>
                            <a href = "../controller/actualizar_estado.php?id=<?php echo $r["nrotramite"];?>" target = "_parent">
                                <center>
                ...
                <center class="paginado">
                    <?php
                        $query = "SELECT c.nrotramite, CONCAT(t.appaterno, ' ', t.apmaterno, ' ', t.nombre) AS nombre_completo, a.nombre AS nombre_asunto, d.nombre AS nombre_dependencia FROM tbtramite c
                                        JOIN tbcliente t ON c.idcliente = t.idcliente JOIN tbasunto a ON c.idasunto = a.idasunto
                                        JOIN tbdependencia d ON c.iddependencia = d.iddependencia
                                        WHERE c.estado = 'Enviado'";
                        $resultado = mysqli_query($cn, $query);
                        $total_registros = mysqli_num_rows($resultado);
                        $total_paginas = ceil($total_registros / $por_pagina);
                        echo "<center><a href = 'expedientesNA.php?pagina=1'>"  . 'Inicio' . " </a>";
                        for ($i = 1; $i <= $total_paginas; $i++) {
                            echo "<a href = 'expedientesNA.php?pagina=" . $i . "'> " . $i . " </a> ";
                        }
                        echo "<a href = 'expedientesNA.php?pagina=$total_paginas'>"  . 'Final' . "</a></center>";
                    ?>
                </center>
            </div>
        </div>
