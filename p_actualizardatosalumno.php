<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!verificarRelacionEscuelaFacultad($idfacultad, $idescuela)) {
            header("location: actualizardatos_alumno.php");
        } else {
            // Actualizar datos en la tabla tbcliente
            $sql = "UPDATE tbcliente SET idescuela = $idescuela, idfacultad = $idfacultad, yearingreso = '$añoingreso', modalidad = '$modalidad', ciclo_grado_nivel = '$ciclo', estado = 1 WHERE email = '$email'";
            mysqli_query($cn, $sql);
